  ...
  <transition name="popup">
    ...
    >
      ...
      <header class="w-full flex justify-between items-center">
        ...
        <button class="sm:text-xl text-sm text-white underline" @click="clearNews">
          Mark as all read
        </button>
      </header>
      <div class="w-full flex flex-col gap-4">
        ...
        >
          <div class="flex gap-6 items-center">
            <div class="flex flex-col gap-1 items-center rounded-full">
              <img
                :src="prefix + notification.user.image"
                alt="profile"
                class="w-20 h-20 sm:w-[60px] sm:h-[60px] rounded-full border-2 border-[#198754]"
              />
...
watch(
  () => props.show,
  (newValue) => {
    if (newValue === false) clearNews()
  }
)

const emits = defineEmits(['clearNews'])
